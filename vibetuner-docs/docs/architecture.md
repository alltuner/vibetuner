...
- Lets `config.css` reference framework templates via
  `@source "node_modules/@alltuner/vibetuner-jinja/templates";` so tailwind
  picks up class names used by base layouts, auth pages, etc.
- The mirror is generated by the package's `prepare` lifecycle hook (no
  manual sync required when developing in the monorepo); registry
  consumers receive a tarball with the templates already baked in.

**Install**: usually `bun add -d @alltuner/vibetuner-jinja` (build-time only)
