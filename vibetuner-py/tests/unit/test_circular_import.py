...
class TestCircularImportPrevention:
    ...
    def test_frontend_submodule_importable_without_load_app_config(self):
        """Importing vibetuner.frontend.oauth must not trigger load_app_config().

        When user route modules import from vibetuner.frontend.oauth (e.g., for
        WebUser or register_oauth_provider), it must not trigger the app factory.
        Otherwise a circular import occurs: __init__ -> load_app_config -> tune.py
        -> user routes -> vibetuner.frontend.oauth -> __init__ (still initializing).

        See: https://github.com/alltuner/vibetuner/issues/1495
        """
        code = textwrap.dedent("""\
            import unittest.mock as mock
            import vibetuner.loader
            ...
            from vibetuner.frontend.oauth import WebUser  # noqa: F401
        ...
        assert "OK" in result.stdout, (
            f"Importing from vibetuner.frontend.oauth triggered load_app_config():\n"
            f"stderr: {result.stderr}"
        )
