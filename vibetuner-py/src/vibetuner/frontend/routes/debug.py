...
# Unprotected router for HMAC-signed debug access
auth_router = APIRouter()


@auth_router.get("/_unlock-debug")
def unlock_debug_access(
    request: Request,
    ts: str | None = None,
    nonce: str | None = None,
    sig: str | None = None,
    ...
    In DEBUG mode, no signature is required.
    In production, validates an HMAC-signed link generated by the CLI.
    """
    if not ctx.DEBUG:
        if ts is None or nonce is None or sig is None:
            raise HTTPException(status_code=404, detail="Not found")

        from vibetuner.debug_signing import verify_debug_signature

        secret = settings.session_key.get_secret_value()
        if not verify_debug_signature(ts=ts, nonce=nonce, sig=sig, secret=secret):
            raise HTTPException(status_code=404, detail="Not found")
