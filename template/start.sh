...
if [ "$1" = "shell" ]; then
    ...
    DEBUG=1 exec uvicorn core.frontend:app --host 0.0.0.0 --port $PORT --reload --reload-dir src/core --reload-dir src/app --reload-dir templates/core/frontend --reload-dir templates/app/frontend --reload-include '*.py' --reload-include '*.html.jinja'
elif [ "$1" = "worker" ]; then
