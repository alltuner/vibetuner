...
class WebUser(BaseUser, BaseModel):
    ...
    name: str
    email: str
    picture: str = DEFAULT_AVATAR_IMAGE
...
def _get_homepage_url(request: Request) -> str:
...
def _create_auth_login_handler(provider_name: str):
    async def auth_login(request: Request, next: str | None = None):
        ...
        request.session["next_url"] = next or _get_homepage_url(request)
        client = oauth.create_client(provider_name)
        if not client:
            return RedirectResponse(url=_get_homepage_url(request))
...
def _create_auth_handler(provider_name: str):
    async def auth_handler(request: Request):
        ...
        try:
            ...
            client = oauth.create_client(provider_name)
            if not client:
                return _get_homepage_url(request)
            ...
            return request.session.pop("next_url", _get_homepage_url(request))
        except OAuthError:
            return _get_homepage_url(request)
